...
| - {ref}`constraint-ip-family`         | &#10003; <br> Valid values: `ipv4` (default), `dual`. `dual` provisions dual-stack (IPv4 + IPv6); requires `load-balancer-sku-name=Standard`. `ipv6` is rejected by Azure (NICs require at least one IPv4 configuration). |
| - {ref}`constraint-mem`                | &#10003;                                       |
