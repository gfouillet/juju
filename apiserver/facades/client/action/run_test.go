...
// enqueueSuite is a test suite used to verify the behavior of operation enqueuing within the API.
type enqueueSuite struct{ MockBaseSuite }

// TestEnqueueSuite runs the runAllSuite tests.
func TestEnqueueSuite(t *stdtesting.T) { tc.Run(t, &enqueueSuite{}) }
...
func (s *enqueueSuite) TestEnqueue_Defaults(c *tc.C) {
	...
	taskArgs := operation.TaskArgs{
		...
		IsParallel:     false, // defaulted to false
